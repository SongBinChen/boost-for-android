...
# BOOST_DOWNLOAD_LINK="https://boostorg.jfrog.io/artifactory/main/release/$BOOST_VER2.$BOOST_VER2.$BOOST_VER3/source/boost_${BOOST_VER1}_${BOOST_VER2}_${BOOST_VER3}.tar.bz2" JFrog service down after 2024/12
...
(
  ...
  if [ -n "$WITH_ICONV" ] || echo $LIBRARIES | grep locale; then
    if [ -e libiconv-libicu-android ]; then
      ...
      git clone --depth=1 -b vxmt-mtcam https://github.com/SongBinChen/libiconv-libicu-android.git || exit 1
    fi
